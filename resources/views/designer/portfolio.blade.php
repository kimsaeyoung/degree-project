...
<div class="animsition" data-animsition-in-class="fade-in-down" data-animsition-in-duration="1000" data-animsition-out-class="fade-out-down" data-animsition-out-duration="800">
    ...
    <div id="DB_banner3">
        <div class="DB_mask">
            <ul class="DB_imgSet">
                ...
                <li><a onclick="slide_img('{{$row->mpf_num}}')" target="_self" style="cursor:pointer"><img class="image_slide" src="{{asset('img/portfolio/'.$row->mpf_picture)}}" alt=""/></a></li>
                @endforeach
            </ul>
        </div>
    ...
    <div class="section">
        <div class="pf_subject">
            <h1 class="h1_pf_subject">{{$port_info[0]->mpf_subject}}</h1>
        </div>
            <!-- web view -->
        <div class="image_box">
        @if($port_info[0]->mpf_division == 1)
        <div class="full_image">
            <figure class="effect-bubba">
                <a href="#">
                    <img class="img" src="{{asset('img/portfolio/'.$port_info[0]->mpf_picture)}}">
                </a>
                <figcaption>
                    @if($s_port_info !=null)
                    <div class="s_img" >
                        @foreach($s_port_info as $row)
                        <a onclick="web_img('{{$row->mpf_num}}','{{$row->pf_num}}')" class="a_tag"><img class="image_slide" src="{{asset('img/portfolio/'.$row->pf_picture)}}"></a>
                        @endforeach
                    </div>
                    @endif
                </figcaption>
        ...
        @elseif($port_info[0]->mpf_division == 2)
        <!-- app image -->
        <div class="appfull_image">
        <figure class="effect-bubba">
                <a href="#">
                    <img class="appfull_image" src="{{asset('img/portfolio/'.$port_info[0]->mpf_picture)}}">
                </a>
                <figcaption>
                    @if($s_port_info !=null)
                    <div class="s_img" >
                        @foreach($s_port_info as $row)
                        <a onclick="app_img('{{$row->mpf_num}}','{{$row->pf_num}}')" class="a_tag"><img class="image_slide" src="{{asset('img/portfolio/'.$row->pf_picture)}}"></a>
                        @endforeach
                    </div>
                    @endif
                </figcaption>
        ...
        <div class="pf_content">
            {{$port_info[0]->mpf_content}}
        </div>
    </div>
    @elseif($port_info != null && $port_info[0]->mpf_division == 2)

    @endif
    
    <!--Body End-->
    @if($m_num == Session::get('m_num'))
    <!--<center>
        <div class="button">
            ...
            <div class="modal-dialog" id="web_dialog">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h3 class="modal-title" id="web_title">{{$port_info[0]->mpf_subject}}</h3>
                    </div>
                    <div class="modal-body">
                        <input type="hidden" value="{{$port_info[0]->mpf_num}}" id="modal_body">
