...
<script>
    ...
    socket.on('note', function(data){
        if('{{ Session::get('m_num') }}' == data.receiver) location.reload();
    });
...
<div class="container">
    ...
    <div class="left_side_menu">
        ...
        <div class="list_box">
            ...
            <label><input class="chk_info" type="radio" name="sender" value="{{$senders[$i]->sender}}" onclick="chk_info({{$senders[$i]->sender}})">{{$senders[$i]->m_name}}</label>
            <?php } } ?>
        </div>
    </div>
    ...
    <div id="msgList">
        ...
        <form class="msgBox" onclick="javascript:readCheck( {{ $results[$i]->msg_num}} , {{$results[$i]->receiver }} , {{$results[$i]->sender}}, '{{$results[$i]->m_name}}');" style="
        @if($results[$i]->checked==0) background-color:lightgray @else background-color:lightblue @endif">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <input type="hidden" id="msg_num" value="{{$results[$i]->msg_num}}"/>
            <input type="hidden" id="receiver" value="{{$results[$i]->receiver}}"/>
            <div class="msg_sender">{{$results[$i]->m_name}}</div>
            <div class="msg_content">{{$results[$i]->msg_content}}</div>
            <div class="msg_date">{{$results[$i]->msg_date}}</div>
        </form>
        <?php } } ?>
    </div>
</div>
