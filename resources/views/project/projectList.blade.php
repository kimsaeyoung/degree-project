...
<div class="container">
    <div id="pTitle">
        <h1>プロジェクトの検索</h1>
        <h5>現在{{$count}}つのプロジェクトが登録されています。</h5>

        <!--프로젝트 검색 버튼-->
        <form id="custom-search-form" class="form-search form-horizontal pull-right" action="{{route('projectList')}}"
              method="get">
        ...
        @if(Session::get('div_member')=='2')
            <a href="{!!route('projectwrite')!!}" id="btn_write" class="btn btn-default"
               style="border-color: hotpink; border-width:2px">
                プロジェクト登録</a>
        @endif
        {{--<a href="{!!route('projectManage')!!}"  id = "btn_write" class="btn btn-default">가등록관리</a>--}}
        ...
        <div id="STATICMENU">
            <div class="left_side_menu">
                ...
                <div class="left_side_menu_a">
                    <div class="left-box">
                        <div class="price_box1"><a href="{!!route('projectList/{post}','high')!!}">金額高い順</a></div>
                        <div class="price_box2"><a href="{!!route('projectList/{post}','low')!!}">金額低い順</a></div>
                    </div>
                    <div class="right-box">
                        <div class="price_box3"><a href="{!!route('projectList/{post}','new')!!}">最新の登録順</a></div>
                        <div class="price_box4"><a href="{!!route('projectList/{post}','deadline')!!}">締め切りの順</a></div>
                    </div>
                </div>
                ...
                {{-- <div class="category-box-title">金額の整列</div>
                 <div id="money">
                ...
                <div class="left_side_menu_c">
                    <form id="area_custom-search-form" class="form-search_a form-horizontal pull-left "
                          action="{{route('projectList')}}" method="get">
        ...
        <div id="main">
            ...
            <div id="search_info">
                @foreach($results as $result)
                    ...
                    <a class="mainList" href="{!!route('projectView/{pj_num}',$result->pj_num)!!}">

                        @if($result->st_num==4)
                            <div class="a_div" style="text-align: center; background: lightgrey; opacity: 1">
                        @elseif($result->st_num==5)
                        ...
                        @endif
                            ...
                            <div class="div_1" style="text-align: left">
                                <div class="db_data_form">{{$result->pj_title}}</div>
                                <div id="pj_edge">
                                    @if($result->st_num==1)
                                        <div class="edge" style="background-color: green;">募集中</div>
                                        @if($result->created_at < $today_add_1)
                                        ...
                                        @endif
                                    @elseif($result->st_num==4)
                                        <div class="edge" style="background-color: darkslategrey;">募集終了</div>
                                    @elseif($result->st_num==5)
                                        <div class="edge" style="background-color: darkgray;">完了</div>
                                    @endif
                                </div>
                                {{--<span class="div_pj_date">登録日&nbsp;:&nbsp;{!!Carbon\Carbon::parse($result->created_at)->format('Y-m-d h:i:s')!!}</span>--}}
                            </div>

                            <div class="div_2" style="text-align: center">
                                <div class="div_2_a">
                                    <span>募集の締切日</span>
                                    <div class="circle">{!!Carbon\Carbon::parse($result->pj_date)->format('m/d')!!}</div>
                                </div>
                                <div class="div_2_a">
                                    <span>予想期間</span>
                                    <div class="circle">{!!$result->expect_date!!}日</div>
                                </div>
                                <div class="div_2_a">
                                    <span>募集人員</span>
                                    <div class="circle">{!!$result->pj_people!!}名</div>
                                </div>
                                <div class="div_2_a">
                                    <span>地  域</span>
                                    <div class="circle">{!!$result->pj_area!!}</div>
                                </div>
                            </div>

                            <div class="div_3">{!!$result->pj_content!!}</div>

                            <div class="div_4">
                                <span class="db_pj_price">予想金額 : <?=number_format($result->pj_price)?>円</span>
                                <span class="glyphicon glyphicon-pencil">要求技術</span>
                                @foreach($pj_tech as $row)
                                    @if($result->pj_num == $row->pj_num )
