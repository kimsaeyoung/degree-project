...
<script>
    ...
    @if(!Session::has('m_num'))
     <script>
         window.alert('ログインした後サービスを利用することができます。');
     </script>
     {!!\Illuminate\Support\Facades\Redirect::route('projectList/nologin')!!}
     @else
    ...
    <div id="wrapper">
            ...
            <h1>
               {!!$pj_view->pj_title!!}
                <span>
                    @if($pj_view->bc_num== 1)
                        ウェブ
                    @elseif($pj_view->bc_num ==2)
                        アプリ
                    @endif{!!'>'!!}{!!$pj_view->sc_name!!}
                </span>
            </h1>
        </div>
        ...
        <div id="info_container">
            <div id="pj_info">
                <div>
                    ...
                    <span>~{!!$pj_view->expect_date!!}日</span>
                </div>
                <div>
                    <span class="glyphicon glyphicon-hourglass">&nbsp;登録の締切日</span>
                    <span>{!!Carbon\Carbon::parse($pj_view->pj_date)->format('Y-m-d')!!}</span>
                </div>
                <div>
                ...
                <div>
                    ...
                    <span>{!!$pj_view->pj_people!!}名</span>
                </div>
                <div>
                    <span class="glyphicon glyphicon-home">&nbsp;会社の位置</span>
                    <span>{!!$pj_view->pj_area!!}</span>
                </div>
                <div>
                    ...
                    <span>ファイルなし</span>
                    @endif
                    <a class="glyphicon glyphicon-download" href="{!!route('projectView/{id}download',$pj_view->pj_file)!!}" download="../storage/public/img/{!!$pj_view->pj_file!!}">{!!$pj_view->pj_file!!}</a>
                </div>      
            </div>
            ...
            <script>
                ...
                <div id='pj_company_logo'>
                    <img src="@if($company_info[0]->m_face) {{asset('img/member_face/'.$company_info[0]->m_face)}} @else {{asset('img/member_face/no_company_face.png')}} @endif" class="img-circle" alt="user_image" width="210" height="195">
                </div>
        ...
        <div id="pj_explain">
            ...
            <pre id="pj_explain_con">
                <textarea id="pj_content" readonly>{!!$pj_view->pj_content!!}</textarea>
            </pre>
        </div>

        <!-- 프로젝트 스마트 매칭 -->
        @if(Session::get('m_num')==$pj_view->m_num)
        <div id="pj_matching">
        ...
        @if($matching)
            <div id="pj_matching_con">
                ...
                <div class="memberInfo">
                    <figure>
                        <img src="@if($index->m_face) {{asset('img/member_face/'.$index->m_face)}} @else {{asset('img/member_face/no_face.jpg')}} @endif" alt=""/>
                        <figcaption>
                            <h2>{{$index->m_name}}</h2>
                            <p>
                                携帯番号<br>
                                ...
                                <input type="hidden" name="_token" value="{{csrf_token()}}">
                                <a class="btn btn-info btn-sm" href="/designer/{{$index->m_num}}">ポートログ</a>
                                <a class="btn btn-info btn-sm" onclick="contractWrite({{$pj_view->pj_num}} , {{$index->m_num}})">契約する</a>
            ...
            <div class="no_smart_matching" >
                <img src ="{{ asset('/img/member_face/smart_matching_no_data.jpg') }} " style="width: 250px">
            </div>
        @endif
        </div>
        ...
        <div id="pj_inquiry">
            <button type="button" class="btn btn-default btn-lg" onclick="window.location='{!!route('projectList')!!}'"> リスト</button>
    ...
    function initialize() {
        var mapLocation = new google.maps.LatLng('{!!$pj_view->company_lat!!}','{!!$pj_view->company_lng!!}'); // 지도에서 가운데로 위치할 위도와 경도
        var markLocation = new google.maps.LatLng('{!!$pj_view->company_lat!!}','{!!$pj_view->company_lng!!}'); // 마커가 위치할 위도와 경도
        ...
        // 말풍선 안에 들어갈 내용
            var content = "会社の名前 : {!!$company->m_id!!}<br>会社のリーダー : {!!$company->cp_represent!!}<br>会社のメール : {!!$company->m_email!!}<br>リーダーの携帯番号 : {!!$company->cp_tel!!}<br>";
    ...
    function showMessage() {
        $('#msgReceiverNum').val({!!$pj_view->m_num!!});
        $('#msgReceiverName').val('{!!$company->cp_represent!!}');
        $('#msgModal').modal();
    }
</script>
